    ...
    internal class ClockCalendarViewModel : ObservableObject
    {
        ...
        public DateTime Date
        {
            ...
            set
            {
                ...
                HeaderDay = DateTime.Now.Year == _date.Year && DateTime.Now.Month == _date.Month
                    ? DateTime.Now.ToString(_moduleConfig.Calendar!.TodayDayFormat)
                    : string.Empty;

                HeaderDate = DateTime.Now.Year == _date.Year && DateTime.Now.Month == _date.Month
                    ? DateTime.Now.ToString(_moduleConfig.Calendar!.TodayDateFormat)
                    : Date.ToString(_moduleConfig.Calendar!.OtherDateFormat);

                LastMonth = Date.AddMonths(-1).ToString(_moduleConfig.Calendar!.OtherMonthFormat);
                NextMonth = Date.AddMonths(+1).ToString(_moduleConfig.Calendar!.OtherMonthFormat);
        ...
        private void UpdateDays()
        {
            ...
            for (int i = Date.Day; i <= DateTime.DaysInMonth(Date.Year, Date.Month); i++)
            {
                ...
                {
                    day.Foreground = Palette.Instance[_config.Flavor].Colors[_moduleConfig.AccentColor].SolidColorBrush;
                }
